...
def get_filtered_protein(protein):
    with open(f"{current_path}/outputs/Megadock_OUT/{protein}_target_input_0/{protein}_scorelist.json", 'r') as f:
...
def group_values(protein):
    ...
    with open(f"/{current_path}/outputs/Megadock_OUT/{protein}_rotate/cluster_5_3_model_final", 'r') as cluster_file:
    ...
    for cluster_line in cluster_lines:
        ...
        if re_line:
            ...
            pizsa_dictionary[cluster_id]=max(pizsa_list)
...
for protein in pdb_listem:
    ...
    #print(score_list)
    os.chdir(input_path)
    with open(f'group_scorelist_filtered_vor+sasa_selected.json', 'w') as f:
    ...
    print(protein)
    print(vorqma_pizsa)
    with open(f'group_rank_aggregation_vor+sasa_selected.json', 'w') as f:
