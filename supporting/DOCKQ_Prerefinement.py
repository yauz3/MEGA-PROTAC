...
for protein in protein_protein_list:
    ...
    predicted=f"{current_path}/Publication_Data_Final/Publication_Data/Prerefinement_Publication_data"
    os.chdir(
        f"/{current_path}/Publication_Data_Final/Publication_Data/Prerefinement_Publication_data/{protein}")
    proteins = glob.glob("*merged*complex*")
    rmsd_list=[]
    for pro in proteins:
        # print(protein)
        try:
            fnat, fnonnat, irms, lrms, dockq = dockq_rmsd.rmsd_calculation(f"{true_ternary}/{protein}_Native.pdb",
                                                                           f"/{current_path}/Publication_Data_Final/Publication_Data/Prerefinement_Publication_data/{protein}/{pro}"
                                                                           )
            if float(dockq) > 0.22:
                print(pro)
                print("fnat", fnat)
